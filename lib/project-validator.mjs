...
function validateSource(source, errors) {
  reportUnknownKeys(source, ["diameter_mm", "spectrum", "position_mm"], "source", errors);
  expectNumber(source, "diameter_mm", "source", errors, { gt: 0 });
  expectString(source, "spectrum", "source", errors);
  if ("position_mm" in source && source.position_mm !== null) {
    if (!isObject(source.position_mm)) {
      pushError(errors, "source.position_mm", "must be an object when provided");
    } else {
      reportUnknownKeys(
        source.position_mm,
        ["x_mm", "y_mm", "z_mm"],
      ...
      expectNumber(source.position_mm, "x_mm", "source.position_mm", errors);
      expectNumber(source.position_mm, "y_mm", "source.position_mm", errors);
      expectNumber(source.position_mm, "z_mm", "source.position_mm", errors);
    }
  }
}
...
function validateBoundary(boundary, errors) {
  ...
  if (type === "ring") {
    ...
    if ("z_m" in boundary) {
      expectNumber(boundary, "z_m", "safety.boundary", errors);
    }
  }
